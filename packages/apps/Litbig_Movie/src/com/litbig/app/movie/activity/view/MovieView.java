...
public abstract class MovieView implements MoviePlayerInterface, MoviePlayerCallbackInterface {
	protected MovieActivity mActivity;

	public MovieView(MovieActivity activity) {
		mActivity = activity;
	}
	...
	public void setSurface(Surface surface) {
		if (null != mActivity.getService()) {
			try {
				mActivity.getService().setSurface(surface);
	...
	public int getPlayState() {
		int playState = MovieUtils.PlayState.STOP;
		if (null != mActivity.getService()) {
			try {
	...
	public int getShuffle() {
		int shuffle = MovieUtils.ShuffleState.OFF;
		if (null != mActivity.getService()) {
			try {
	...
	public int getRepeat() {
		int repeat = MovieUtils.RepeatState.OFF;
		if (null != mActivity.getService()) {
			try {
	...
	public Bitmap getVideoThumbnail(int index, boolean isNowPlaying, boolean isScale) {
		...
		if (null != mActivity.getService()) {
			try {
				videoThumbnail = mActivity.getService().getVideoThumbnail(index, isNowPlaying, isScale);
			} catch (RemoteException e) {
				e.printStackTrace();
			}
		}
	...
	public int getNowPlayingCategory() {
		int category = MovieUtils.Category.ALL;
		if (null != mActivity.getService()) {
			try {
